...
\begin{tikzpicture}[%
	...
	\node[multidoc, right=of emr](con){Danh sách khái niệm};
	...
	\node[io, right=3cm of con](perp){Đặc trưng Person-Person};
	\node[io](pati){Đặc trưng Person};
	\node[io](prop){Đặc trưng Problem-Problem};
	\node[io](tstp){Đặc trưng Test-Test};
	\node[io](trep){Đặc trưng Treatment-Treatment};
	...
	\node[io](patim){SVM model Patient-class};
...
\begin{tikzpicture}[%
	...
	\node[doc](emr){EMR};
	\node[doc, right=of emr](con){Danh sách khái niệm};
	\node[proc, below=1.3cm of $(emr)!0.5!(con)$](prep){Tiền xử lý};
