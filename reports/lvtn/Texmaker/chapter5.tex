...
\begin{tikzpicture}[%
	...
	\node[io,right=7cm of perp](perm){SVM model cho cặp Person};
	\node[io](patim){Mô hình SVM Patient-class};
	\node[io](prom){Mô hình SVM cho cặp Problem};
	\node[io](tstm){Mô hình SVM cho cặp Test};
	\node[io](trem){Mô hình SVM cho cặp Treatment};
	\node[io](pronm){Mô hình SVM cho Pronoun};
...
\item Phương pháp này xây dựng các mẫu huấn luyện theo các bước sau: với mỗi hồi chỉ $C_j$, xác định tiền đề của nó ở xa nhất theo thứ tự xuất hiện, gọi nó là $C_k$. Xây dựng các cặp với khái niệm đứng sau là $C_j$, khái niệm đứng trước là các khái niệm bắt đầu từ $C_k$ đến $C_{j-1}$ (chỉ xét những cặp hai khái niệm cùng lớp) \cite{VincentNg2002[2]}. Như vậy đối với ví dụ trên Hình \ref{fig:insgen-eg}, các mẫu  huấn luyện được sinh ra theo các bước phương pháp này là: đối với $C_3$, tiền đề xa nhất của nó là $C_1$, như vậy các cặp $(C_1,\,C_3)$, $(D_1,\,C_3)$, $(D_2,\,C_3)$, $(a,\,C_3)$, $(b,\,C_3)$ và $(C_2,\,C_3)$ được xây dựng. Làm tương tự cho các khái niệm từ $C_2$ về $C_1$ để xây dựng các mẫu huấn luyện còn lại.
\end{enumerate}

\section{Rút trích đặc trưng lớp Person}
...
\begin{newtable}{1.2}{Tập đặc trưng cho lớp Person\label{tab:PersonFeatures}}
%\centering\ra{1.2}
...
First one/two/three words before mention & & Một/hai/ba từ liền trước khái niệm đang xét\\
First one/two/three words after mention & & Một/hai/ba từ liền sau khái niệm đang xét\\
First one/two/three words in between mention, problem & & Một/hai/ba từ đầu tiên nằm giữa khái niệm đang xét và khái niệm lớp Problem\\
Last one/two/three words in between mention, problem & & Một/hai/ba từ cuối cùng nằm giữa khái niệm đang xét và khái niệm lớp Problem\\
First one/two/three words in between mention, treatment & & Một/hai/ba từ đầu tiên nằm giữa khái niệm đang xét và khái niệm lớp Treatment\\
Last one/two/three words in between mention, treatment & & Một/hai/ba từ cuối cùng nằm giữa khái niệm đang xét và khái niệm lớp Treatment\\
First one/two/three words in between mention, test & & Một/hai/ba từ đầu tiên nằm giữa khái niệm đang xét và khái niệm lớp Test\\
Last one/two/three words in between mention, test & & Một/hai/ba từ cuối cùng nằm giữa khái niệm đang xét và khái niệm lớp Test\\
\bottomrule
\end{tabularx}
\end{table}
