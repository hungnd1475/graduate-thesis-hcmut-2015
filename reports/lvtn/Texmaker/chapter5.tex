...
Chi tiết thuật toán gom cụm tốt nhất trước, BEST-CLUSTER, được trình bày ở Hình \ref{best-cluster-algor}. BEST-CLUSTER nhận đầu vào là một văn bản HSXV, danh sách các khái niệm đã được gán nhãn của văn bản này và xuất ra danh sách các chuỗi đồng tham chiếu của nó.
...
\SetKwFunction{ExtractFeature}{ExtractFeature}
\SetKwFunction{Classify}{Classify}
\SetKwFunction{IsCoref}{IsCoref}
\SetKwFunction{Confidence}{Confidence}

\Input{văn bản HSXV $E$, danh sách khái niệm $C$}
\Output{danh sách chuỗi đồng tham chiếu của $E$}
\BlankLine
$M$: số khái niệm\;
...
\For{$i \leftarrow 1$ \KwTo $M$}{
	...
	\eIf{\TypeOf{$ana$} $\neq$ \PRONOUN}{
		...
		\For{$j \leftarrow i - 1$ \KwDownTo 0}{
			...
			\BlankLine
			$f \leftarrow \ExtractFeature{E, ante, ana}$\;
			$r \leftarrow \Classify{f}$\;
			
			\If{\IsCoref{$r$}}{
				\If{$bestConf = \Null$ \KwOr $bestConf < \Confidence{r}$}{
					$confidence \leftarrow \Confidence{r}$\;
		...
		\If{$bestConf \neq \Null$ \KwAnd $bestAnte \neq \Null$}{
			$pairs \leftarrow pairs \cup (bestAnte, ana)$\;
		}
	}{	
		$f \leftarrow \ExtractFeature{E, ana}$\;
		$r \leftarrow \Classify{f}$\;
		$type \leftarrow \TypeOf{r}$\;
		\BlankLine
		\For{$j \leftarrow i - 1$ \KwDownTo 0}{
			$ante \leftarrow C[j]$\;
			\If{$\TypeOf{ante} = type$}{
				$pairs \leftarrow pairs \cup (ante, ana)$\;
				\KwBreak\;
			}
		}
	}	
...
\end{algorithm}
\caption{Giải thuật gom cụm tốt nhất trước \label{best-cluster-algor}}
\end{figure}
