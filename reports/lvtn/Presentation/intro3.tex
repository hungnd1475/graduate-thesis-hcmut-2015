...
\midrule
\tikz\node[na, inner ysep=0mm](Person){\textit{\color<3>{red}Person}}; & Các khái niệm chỉ người & the patient, Dr. John\\
\textit{\color<3>{red}Problem} & Các vấn đề về sức khỏe & cancer, pain\\
\textit{\color<3>{red}Test} & Các thủ tục y tế & blood pressure\\
\tikz\node[na, inner ysep=0mm](Treatment){\textit{\color<3>{red}Treatment}}; & Các phương pháp điều trị & anti-biotic\\
\textit{Pronoun} & Các đại từ & which, who, that\\
\bottomrule
\uncover<3>{
\begin{tikzpicture}[overlay]
\coordinate[left=.2cm of $(Person.west)!0.5!(Treatment.west)$](tmp-node);
\path (Treatment.south west) edge[out=150,in=330] (tmp-node);
\path (tmp-node) edge[out=30,in=210] (Person.north west);
\node[na,text width=0.5cm,left=.3cm of tmp-node]{Lớp chính};
\end{tikzpicture}
\end{tabular}
}
}
