...
namespace HCMUT.EMRCorefResol.ConsoleTest
{
    ...
    class Program
    {
        ...
        {
            var emr = new EMR(emrFile, conceptsFile, medicationFile, new I2B2DataReader());
        ...
        static void testFeatures()
        {
            ...
            var emr = new EMR(emrFile, conceptsFile, medicationFile, reader);
        ...
        static string testTrainer()
        {
            ...
            TrainingSystem.Instance.TrainOne(EMR_COLLECTION.GetEMRPath(0), EMR_COLLECTION.GetConceptsPath(0),
                EMR_COLLECTION.GetChainsPath(0), EMR_COLLECTION.GetMedicationsPath(0), dataReader, preprocessor, fExtractor, trainer);
        ...
        static string testTrainManyEMR(int size)
        {
            ...
            string[] emr, concepts, chains, medications;
            EMR_COLLECTION.GetRandom(size, out emr, out concepts, out chains, out medications);
            TrainingSystem.Instance.TrainAll(emr, concepts, chains, medications, dataReader, preprocessor, fExtractor, trainer);
        ...
        static void testClassifier(IClassifier classifier, int size)
        {
            ...
            string[] emrPaths, conceptsPaths, chainsPaths, medications;
            EMR_COLLECTION.GetRandom(size, out emrPaths, out conceptsPaths, out chainsPaths, out medications);

            Console.WriteLine("\n====Testing Classifier====\n");
            for (int i = 0; i < size; i++)
            {
                ClassificationSystem.Instance.ClassifyOne(emrPaths[i], conceptsPaths[i], chainsPaths[i], medications[i],
        ...
        static void testClassifier(IClassifier classifier, EMRCollection emrColl)
        {
            ...
            while (true)
            {
                ClassificationSystem.Instance.ClassifyOne(emrColl.GetEMRPath(emrIndex),
                    emrColl.GetConceptsPath(emrIndex), emrColl.GetChainsPath(emrIndex), emrColl.GetMedicationsPath(emrIndex),
                    dataReader, preprocessor, fExtractor, classifier);
                classifier.ClearCache();
